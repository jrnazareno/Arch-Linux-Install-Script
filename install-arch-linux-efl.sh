...
PACKAGES() {
	pacman -S --noconfirm --needed connman bluez vim base-devel linux-headers bash-completion xorg efl enlightenment lightdm lightdm-gtk-greeter terminology firefox > /dev/null 2>&1 && systemctl enable connman > /dev/null 2>&1 && systemctl enable lightdm > /dev/null 2>&1  
}
