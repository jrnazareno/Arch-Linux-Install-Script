...
echo ""
echo ""
echo "ADDITIONAL NOTES   2021-10-21"
echo "(J. Nazareno)"
echo ""
echo "my custom steps before running this script:"
echo "CREATE PARTITIONS"
echo "  fdisk /dev/sda"
echo "  /dev/sda1  512M             UEFI "
echo "  /dev/sda2  1GB              Linux swap"
echo "  /dev/sda3  remaining space  Linux"
...
PACKAGES() {
	pacman -S --noconfirm --needed networkmanager vim base-devel linux-headers bash-completion xorg efl enlightenment lightdm lightdm-gtk-greeter lightdm-webkit-theme-litarvan terminology firefox > /dev/null 2>&1 && systemctl enable NetworkManager > /dev/null 2>&1 && systemctl enable lightdm > /dev/null 2>&1 
}
