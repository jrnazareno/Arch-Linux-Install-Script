...
PACKAGES() {
	pacman -S --noconfirm --needed networkmanager vim base-devel linux-headers bash-completion xorg plasma sddm > /dev/null 2>&1 && systemctl enable NetworkManager > /dev/null 2>&1 && systemctl enable gdm > /dev/null 2>&1
}
