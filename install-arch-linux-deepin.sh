...
PACKAGES() {
	pacman -S --noconfirm --needed networkmanager vim base-devel linux-headers bash-completion xorg deepin lightdm lightdm-gtk-greeter firefox > /dev/null 2>&1 && systemctl enable NetworkManager > /dev/null 2>&1 && systemctl enable lightdm > /dev/null 2>&1
}
