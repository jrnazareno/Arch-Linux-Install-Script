...
PACKAGES() {
	pacman -S --noconfirm --needed networkmanager vim base-devel linux-headers bash-completion sway swaylock swayidle dmenu xorg-xwayland alacritty terminator firefox > /dev/null 2>&1 && systemctl enable NetworkManager > /dev/null 2>&1 
}
